...
def generator_all(
    ...
    real_noncandidate_files,
    KO_candidate_files,
    KO_noncandidate_files,
    batch_size,
    bin=0.2,
    binsample=200,
    WT_label=1,
        ...
        if os.path.isfile(KO_noncandidate):
            ...
            if len(temp["X"]) != 0:
                ...
                select_index = np.random.choice(
                    np.arange(0, temp_size, int(temp_size / 10000))
                )
                index_noncandidate_KO.append(select_index)
                print(
                    "debug "
                    + KO_noncandidate
                    + " index_noncandidate_KO="
                    + str(select_index)
                    + "\n"
                )
                index += 1
    ...
    batch_size_KO_noncandidate = int(
        batch_size * KO_noncandidate_weight / len(KO_noncandidate_h5fs)
    )
    ...
    for datatype in range(
        len(real_noncandidate_files.split(";"))
    ):  # use ; for different datatype
        (
            real_noncandidate_h5fs[datatype],
            real_noncandidate_samplesize[datatype],
            ...
            if os.path.isfile(real_noncandidate):
                ...
                if len(temp["X"]) == 0:
                    ...
                    print(
                        "debug "
                        + real_noncandidate
                        + " index_noncandidate_real="
                        + str(select_index)
                        + "\n"
                    )
                    index += 1

        batch_size_real_noncandidate[datatype] = int(
            batch_size
            * real_noncandidate_weight
            / len(real_noncandidate_h5fs[datatype])
        )
        print(
            "real_noncandidate_file_num=" + str(len(real_noncandidate_h5fs[datatype]))
        )
    ...
    real_candidate_file_size = [
        int(np.ceil(x / minsize)) for x in real_candidate_file_size
    ]
    ...
    for start in ratio_start:
        ...
        for _ in range(real_candidate_file_num):
            index_start_end_WT = np.where(
                (AG_ratio_WT[_] <= end) & (AG_ratio_WT[_] > start)
            )[0]
            AG_ratio_WT_index[start].append(
                index_start_end_WT
            )  # AG_ratio_WT_index[start] element could be []
    ...
    for start in ratio_start:
        ...
        for _ in range(KO_candidate_file_num):
            index_start_end_KO = np.where(
                (AG_ratio_KO[_] <= end) & (AG_ratio_KO[_] > start)
            )[0]
    ...
    for ratiokey in AG_ratio_WT_index:
        ...
        for datatype_index in range(len(AG_ratio_WT_index[ratiokey])):
            ...
            if (
                len(AG_ratio_WT_index[ratiokey][datatype_index])
                > max_WT_sample_perdatatype[datatype_index]
            ):
                max_WT_sample_perdatatype[datatype_index] = len(
                    AG_ratio_WT_index[ratiokey][datatype_index]
                )
    ...
    for ratiokey in AG_ratio_KO_index:
        ...
        for datatype_index in range(len(AG_ratio_KO_index[ratiokey])):
            ...
            if (
                len(AG_ratio_KO_index[ratiokey][datatype_index])
                > max_KO_sample_perdatatype[datatype_index]
            ):
                max_KO_sample_perdatatype[datatype_index] = len(
                    AG_ratio_KO_index[ratiokey][datatype_index]
                )
    ...
    while 1:
        for datatype in range(real_candidate_file_num):
            for time_ in range(real_candidate_file_size[datatype]):
                ...
                sample_weights = []
                # IVT data start
                x_TN_batch = IVT_neg_h5fs["X"][
                    index_IVT_TN : (index_IVT_TN + batch_size),
                    win_center_index - nt : win_center_index + nt + 1,
                    :featuredim,
                ]
                y_ref_TN_batch = IVT_neg_h5fs["y_ref"][
                    index_IVT_TN : (index_IVT_TN + batch_size)
                ]
                y_call_TN_batch = IVT_neg_h5fs["y_call"][
                    index_IVT_TN : (index_IVT_TN + batch_size)
                ]
                sample_weights.extend(np.ones(len(x_TN_batch)))
                x_TP_batch = IVT_pos_h5fs["X"][
                    index_IVT_TP : (index_IVT_TP + batch_size),
                    win_center_index - nt : win_center_index + nt + 1,
                    :featuredim,
                ]
                y_ref_TP_batch = IVT_pos_h5fs["y_ref"][
                    index_IVT_TP : (index_IVT_TP + batch_size)
                ]
                y_call_TP_batch = IVT_pos_h5fs["y_call"][
                    index_IVT_TP : (index_IVT_TP + batch_size)
                ]
                ...
                for _ in range(len(KO_noncandidate_h5fs)):
                    ...
                    x_KO_batch.append(
                        KO_noncandidate_h5fs[_]["X"][
                            index_noncandidate_KO[_] : (
                    ...
                    y_ref_KO_batch.append(
                        KO_noncandidate_h5fs[_]["y_ref"][
                            index_noncandidate_KO[_] : (
                                index_noncandidate_KO[_] + batch_size_KO_noncandidate
                            )
                        ]
                    )
                    y_call_KO_batch.append(
                        KO_noncandidate_h5fs[_]["y_call"][
                            index_noncandidate_KO[_] : (
                                index_noncandidate_KO[_] + batch_size_KO_noncandidate
                            )
                        ]
                ...
                (
                    X_real_noncandidate,
                    y_ref_real_noncandidate,
                    y_call_real_noncandidate,
                ) = ([], [], [])
                for _ in range(len(real_noncandidate_h5fs[datatype])):
                    X_real_noncandidate.append(
                        real_noncandidate_h5fs[datatype][_]["X"][
                            index_noncandidate_real[datatype][_] : (
                                index_noncandidate_real[datatype][_]
                                + batch_size_real_noncandidate[datatype]
                            ),
                            win_center_index - nt : win_center_index + nt + 1,
                            :featuredim,
                        ]
                    )
                    y_ref_real_noncandidate.append(
                        real_noncandidate_h5fs[datatype][_]["y_ref"][
                            index_noncandidate_real[datatype][_] : (
                                index_noncandidate_real[datatype][_]
                                + batch_size_real_noncandidate[datatype]
                            )
                    ...
                    y_call_real_noncandidate.append(
                        real_noncandidate_h5fs[datatype][_]["y_call"][
                            index_noncandidate_real[datatype][_] : (
                                index_noncandidate_real[datatype][_]
                                + batch_size_real_noncandidate[datatype]
                            )
                    ...
                    if (
                        ...
                        print(
                            "debug real noncandidate to the end size. in file "
                            + real_noncandidate_files.split(";")[datatype].split(",")[_]
                            + "\n"
                        )
                ...
                if binselecttype == 1:
                    ...
                    ratio_select = np.random.choice(ratio_start, 1, replace=False)[
                        0
                    ]  # select one ratio from ratio list
                elif binselecttype == 2:  # use start ratio to select bin
                    print("binselecttype==2 select from 1-ratio_start\n")
                    ratio_select = np.random.choice(
                        ratio_start,
                        1,
                        replace=False,
                        p=softmax(1 - np.asarray(ratio_start)),
                    )[
                        0
                    ]  # select one ratio from ratio list
                elif (
                    binselecttype == 3
                ):  # use samplenumbers to select bin, bin with more samples, has high posibility to be selected
                    print("binselecttype==3 select from samplenum\n")
                    ratio_select = np.random.choice(
                        ratio_start,
                        1,
                        replace=False,
                        p=softmax([np.log10(x) for x in ratio_bin_samplenum[datatype]]),
                    )[
                        0
                    ]  # select one ratio from ratio list
                elif binselecttype == 4:  # uniform but select in an order
                    print(
                        "binselecttype==4 select from uniform but in increased order\n"
                    )
                ...
                x = np.concatenate(
                    [x_TN_batch, x_TP_batch, x_KO_batch, X_real_noncandidate, x_real],
                    axis=0,
                )
                ...
                mod_label = np.concatenate(
                    [
                        np.zeros(len(x_TN_batch)),
                        np.ones(len(x_TP_batch)),
                        np.zeros(len(x_KO_batch)),
                        np.zeros(len(X_real_noncandidate)),
                        mod_label_real,
                    ],
                    axis=0,
                )
                ratio_label = np.concatenate(
                    [
                        np.zeros(len(x_TN_batch)),
                        np.ones(len(x_TP_batch)) + 0.1,
                        np.zeros(len(x_KO_batch)),
                        np.zeros(len(X_real_noncandidate)),
                        ratio_label_real,
                    ],
                ...
                if sample_weights_flag:
                    yield (
                        [
                            x,
                            np.expand_dims(y_ref, axis=-1),
                            np.expand_dims(y_call, axis=-1),
                        ],
                        [
                            np.expand_dims(y_ref, axis=-1),
                            np.expand_dims(y_call, -1),
                            np.expand_dims(ratio_label, -1),
                            np.expand_dims(ratio_label, -1),
                        ],
                        [
                            sample_weights_base,
    ...
    while 1:
        for datatype in range(real_candidate_file_num):
            for time_ in range(real_candidate_file_size[datatype]):
                ...
                if len(x_KO_batch) > 0:
                    x = np.concatenate(
                        [x_KO_batch, X_real_noncandidate, x_real], axis=0
                    )
                    y_ref = np.concatenate(
                        [y_ref_KO_batch, y_ref_real_noncandidate, y_ref_real], axis=0
                    )
                    y_call = np.concatenate(
                        [y_call_KO_batch, y_call_real_noncandidate, y_call_real], axis=0
                    )
                    mod_label = np.concatenate(
                        [
                            np.zeros(len(x_KO_batch)),
                    ...
                    y_ref = np.concatenate(
                        [y_ref_real_noncandidate, y_ref_real], axis=0
                    )
                    y_call = np.concatenate(
                        [y_call_real_noncandidate, y_call_real], axis=0
                    )
                    mod_label = np.concatenate(
                        [np.zeros(len(X_real_noncandidate)), mod_label_real], axis=0
                    )
                    ratio_label = np.concatenate(
                        [np.zeros(len(X_real_noncandidate)), ratio_label_real], axis=0
                    )
...
def generator_IVT_shuffle(
    ...
    while 1:
        ...
        mod_label = np.concatenate(
            [np.zeros(len(x_TN_batch)), np.ones(len(x_TP_batch)) + 0.1], axis=0
        )
        ratio_label = np.concatenate(
            [np.zeros(len(x_TN_batch)), np.ones(len(x_TP_batch)) + 0.1], axis=0
        )
        yield (
            [x, np.expand_dims(y_ref, axis=-1), np.expand_dims(y_call, axis=-1)],
            [
                np.expand_dims(y_ref, axis=-1),
                np.expand_dims(y_call, -1),
                mod_label,
    ...
    for hdf5file in hdf5files:
        ...
        for ndx in range(0, total_size, batch_size):
            ...
            for model in modellist:
                predicty = model.model.predict(
                    [batch_data, fake_input, fake_input], batch_size=batch_size
                )
            ...
            if printref:
                for pindex in range(len(predict_mod)):
                    output.write(
                        ...
                        + str(
                            "".join(
                                [encoding_baseout[x] for x in batch_y_ref[pindex][:-2]]
                            )
                        )
                        + "\t"
                        + str(predict_mod[pindex])
                        + "\n"
                    )
            else:
                for pindex in range(len(predict_mod)):
                    if len(batch_info[pindex]) == 0:
                        print(
                            "debug batch_info error in file "
                            + hdf5file
                            + " ndx="
                            + str(ndx)
                            + " pindex="
                    ...
                    output.write(
                        batch_info[pindex] + "\t" + str(predict_mod[pindex]) + "\n"
                    )
...
def ratio_loss(y_true, y_pred):
    ...
    mask = K.any(
        tf.concat([K.equal(y_true, mask_value_0), K.equal(y_true, mask_value_1)], -1),
        axis=-1,
    )  # mask samples that have 0 or 1 labels
...
def pos_ratio_loss(y_true, y_pred):
    ...
    def pos_ratio_loss_temp():
        ...
        return K.abs(y_pred_pos_sum - K.sum(mask_keep1) * mask_value_1) / K.sum(
            mask_keep1
        )
...
def self_rank_loss(y_true, y_pred):  # y_true is ratio
    ...
    def false_fn():
        ...
        def pseudolabel():
            y_pseudo_label = K.greater_equal(
                y_pred_WTmasked, tf.math.top_k(y_pred_WTmasked, WT_numbers).values[-1]
            )
            y_pseudo_label = K.cast(y_pseudo_label, K.floatx()) + tf.squeeze(y_true) * (
                1 - mask
            )
    ...
    y_pseudo_label = K.clip(
        y_pseudo_label, 0, 1
    )  # for IVT data y_true = 1.1 clip it to 1
...
class multihead_attention:
    ...
    def __init__(
        self, max_len, maxoutputlen, featuredim, nb_classes, save_path, kfold_index
    ):
    ...
    def build_model_seq2seq(
        ...
        LSTM2 = Dropout(drop_cnn, name="embeddinglayer")(
            BatchNormalization()(
                Bidirectional(
                    LSTM(
                        dim_lstm,
        ...
        if is_monotonic:
            ...
            preds1 = AttentionDecoder(
                ...
                [LSTM2, outp_true_ref],
                use_teacher_forcing=teacherforcing,
                teacher_forcing_ratio=teacher_forcing_ratio,
            )

        if is_monotonic:
            preds2 = AttentionDecoder(
                name="called_base",
                units=dim_attention,
                alphabet_size=alphabet_size,
                embedding_dim=embedding_output_dim,
            ...
            preds2 = AttentionDecoder(
                ...
                [LSTM2, outp_true_call],
                use_teacher_forcing=teacherforcing,
                teacher_forcing_ratio=teacher_forcing_ratio,
            )

        att = Attention(
            hidden=LSTM2.get_shape()[-1].value,
            da=dim_attention,
            r=headnum,
            init="glorot_uniform",
            activation="tanh",
        ...
        Gslice = Lambda(
            lambda x, i, j: x[:, i:j], output_shape=[1], name="ratio_pred"
        )  # Define your lambda layer
        Gslice.arguments = {"i": 1, "j": 2}  # Update extra arguments to Gslice
        # Call F
        preds3_ratio = Gslice(preds3)
        self.model = Model(
            inputs=[input, outp_true_ref, outp_true_call],
            outputs=[preds1, preds2, preds3, preds3_ratio],
        )
    ...
    def train(
        ...
        batch_size=1000,
        epochs=100,
        val_real_candidate_files=None,
        val_real_noncandidate_files=None,
        val_KO_candidate_files=None,
        val_KO_noncandidate_files=None,
        val_IVT_pos_file=None,
        val_IVT_neg_file=None,
        early_stop=0,
        bin=0.2,
        binsample=100,
        traintype="basecall",
        ...
        )  # use the smallest dataset (real data) as sample size!
        ...
        if early_stop != 0:
            if traintype == "basecall":
                early_stopping = EarlyStopping(
                    monitor=val_prefix + "loss", patience=early_stop
                )
                ...
                early_stopping = EarlyStopping(
                    monitor=val_prefix + "mod_pred_loss", patience=early_stop
                )
        ...
        if traintype == "basecall":
            ...
            model_checkpoint = ModelCheckpoint(
                best_model_path,
                monitor=val_prefix + "mod_pred_loss",
                save_best_only=True,
                verbose=1,
            )
        elif traintype == "modloss":
        ...
        model_checkpoint_everystep = ModelCheckpoint(
            os.path.join(OUTPATH, "model_current.ckpt"), save_best_only=False, verbose=1
        )

        if IVT_pos_file is None or not os.path.exists(IVT_pos_file):
            generator_fun = generator_realdata
        else:
            if real_candidate_files is None or not os.path.exists(
                real_candidate_files.split(",")[0]
            ):
                if shuffleIVT:
                ...
                generator_fun = (
                    generator_all  # generator_IVT_data#generator_IVT_data_nonshuffle
                )
        ...
        if val_real_candidate_files is None:
            ...
            if early_stop != 0:
                history = self.model.fit_generator(
                    generator_fun(
                        ...
                        binsample=binsample,
                        WT_label=WT_label,
                        binselecttype=binselecttype,
                        sample_weights_flag=sample_weights_flag,
                        featuredim=self.featuredim,
                        windowsize=self.max_len,
                        weight_exp=weight_exp,
                        KO_noncandidate_weight=KO_noncandidate_weight,
                        real_noncandidate_weight=real_noncandidate_weight,
                    ),
                ...
                history = self.model.fit_generator(
                    ...
                    epochs=epochs,
                    verbose=1,
                    callbacks=[model_checkpoint, model_checkpoint_everystep],
                    shuffle=True,
                )
        elif early_stop != 0:
            ...
            history = self.model.fit_generator(
                ...
                shuffle=True,  # validation_data=[[x_valid,val_fake_input,val_fake_input],[np.expand_dims(y_valid_ref,axis=-1),np.expand_dims(y_valid_call,axis=-1),val_mod_label,np.expand_dims(val_ratio_label,-1)]]
                validation_data=generator_fun(
                    val_IVT_pos_file,
                    val_IVT_neg_file,
                    val_real_candidate_files,
                    val_real_noncandidate_files,
                    val_KO_candidate_files,
                    val_KO_noncandidate_files,
                    batch_size=batch_size,
                    bin=bin,
                    binsample=binsample,
                    WT_label=WT_label,
