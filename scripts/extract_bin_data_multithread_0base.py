    ...
    for i,j_ref in aligned: #i and j_ref are 0 based
        ...
        if candidate_file is not None:
            if candidateonly==1:
                ...
                if chrom+"-"+str(j_ref+1) not in candidate_sites:
                    continue
            else:
                #  print("noncandidate only\n")
                if chrom+"-"+str(j_ref+1) in candidate_sites:
                    continue
...
def process_signal_for_one_read(read_id,mole,summary_one_read,read_information,ref_chr,sam_info):
    ...
    for i in seq_frags_call_one_read:# here i is the position on ref
        ...
        if candidate_file is not None and candidateonly==1:
             info_ = "\t".join([labeltype+":"+str(candidate_sites[chrom+"-"+str(i)]),read_id,chrom,str(i),strand])
        else:
...
def process_signals(args):
    os.nice(10)
    msgs_queue,eventIn,start_file_pos,start_read_index,num_reads_assigned = args
...
if __name__ == "__main__":
    ...
    for offset,start_read_index in list_of_offsets:
        list_of_args.append((msgs_queue,eventIn,offset,start_read_index,chunksize))
