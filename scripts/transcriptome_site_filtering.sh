...
awk -F '\t' -v OFS="\t" 'ARGIND==1{hash_cdna[$1"\t"$2]++}ARGIND==2{if($1"\t"$2 in hash_cdna){ hash_cdna_site[$1"\t"$2]=$3"\t"$4; hash_cdna_strand[$1"\t"$2]=$5;}}ARGIND==3{hash_gene[$2]=$1}ARGIND==4{site=hash_cdna_site[$1"\t"$2]; print $0,site,hash_gene[$1],hash_cdna_strand[$1"\t"$2]; }' $site_bed $cdna2genome_tab $annotation_gpd $site_bed > $site_tab.genome

if test -s $alu_bed; 
then #filter by ALU
   echo "filtering by ALU"
   awk -F '\t' -v OFS="\t" '{if ($9) print $8,$9,$9+1}' $site_tab.genome | sort -k1,1 -k2,2n | uniq > $temp_bed_1
   ...
   awk -F '\t' -v OFS="\t" -v in_alu_coverage=$in_alu_coverage -v out_alu_coverage=$out_alu_coverage -v in_alu_ratio=$in_alu_ratio -v out_alu_ratio=$out_alu_ratio 'ARGIND==1{hash_alu[$1"\t"$2]++}ARGIND==2{site=$7"\t"$8; if(site in hash_alu && $4>=in_alu_coverage && $5>=in_alu_ratio) print $0; else if($4>=out_alu_coverage && $5>=out_alu_ratio) print $0; }' $temp_bed_2 $site_tab.genome > $site_tab.alu_filtered
    ...
    awk -F '\t' -v OFS="\t" -v coverage_cutoff=$coverage_cutoff -v ratio_cutoff=$ratio_cutoff 'ARGIND==1{ if($4>=coverage_cutoff && $5>=ratio_cutoff) print $0; }' $site_tab.genome > $site_tab.alu_filtered
fi
...
then
    ...
    mv $site_tab $site_tab.temp
    awk -F '\t' -v OFS="\t" 'ARGIND==1{ if($13=="non_snp") print $0 }' $site_tab.temp > $site_tab
    rm $site_tab.temp
fi
...
then
    ...
    mv $site_tab $site_tab.temp
    awk -F '\t' -v OFS="\t" 'ARGIND==1{ if($14=="non_m6A_motif") print $0 }' $site_tab.temp > $site_tab
    rm $site_tab.temp
fi
