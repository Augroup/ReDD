...
awk -F '\t' -v OFS="\t" 'ARGIND==1{hash_cdna[$1"\t"$2]++}ARGIND==2{if($1"\t"$2 in hash_cdna){ hash_cdna_site[$1"\t"$2]=$3"\t"$4; hash_cdna_strand[$1"\t"$2]=$5; hash_site[$3"\t"$4]++}}ARGIND==3{hash_gene[$2]=$1}ARGIND==4{site=hash_cdna_site[$1"\t"$2]; print $0,site,hash_gene[$1],hash_cdna_strand[$1"\t"$2]; }' $site_bed $cdna2genome_tab $annotation_gpd $site_bed > $site_tab.temp
