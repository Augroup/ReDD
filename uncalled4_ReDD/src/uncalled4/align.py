...
class HDF5File():
    def __init__(self,filename,prms):
        ...
        if self.prms.redd_candidate is None:
            self.h5f = h5py.File(filename+'.hdf5', 'w')
            self.h5f_candidate = None
        else:
            self.h5f = h5py.File(filename+'.noncandidate.hdf5', 'w')
            self.h5f_candidate = h5py.File(filename+'.candidate.hdf5', 'w')
        ...
        self.h5f.create_dataset('X', (0,windowsize,featuredim),maxshape=(None,windowsize,featuredim), dtype='float32',chunks=(8192, windowsize,featuredim), compression=hdf5plugin.LZ4())#features has 3+4 
        self.h5f.create_dataset('y_ref', (0,windowsize),maxshape=(None,windowsize), dtype='int',chunks=(8192, windowsize), compression=hdf5plugin.LZ4())
        self.h5f.create_dataset('y_call', (0,windowsize),maxshape=(None,windowsize), dtype='int',chunks=(8192, windowsize), compression=hdf5plugin.LZ4())
        self.h5f.create_dataset('info', (0,),maxshape=(None,), dtype=h5py.special_dtype(vlen=str),chunks=(8192, ), compression=hdf5plugin.LZ4())#features has 3+4 
        if self.prms.redd_candidate is not None:
            self.h5f_candidate.create_dataset('X', (0,windowsize,featuredim),maxshape=(None,windowsize,featuredim), dtype='float32',chunks=(8192, windowsize,featuredim), compression=hdf5plugin.LZ4())#features has 3+4 
            self.h5f_candidate.create_dataset('y_ref', (0,windowsize),maxshape=(None,windowsize), dtype='int',chunks=(8192, windowsize), compression=hdf5plugin.LZ4())
            self.h5f_candidate.create_dataset('y_call', (0,windowsize),maxshape=(None,windowsize), dtype='int',chunks=(8192,windowsize), compression=hdf5plugin.LZ4())
            self.h5f_candidate.create_dataset('info', (0,),maxshape=(None,), dtype=h5py.special_dtype(vlen=str),chunks=(8192, ), compression=hdf5plugin.LZ4())#features has 3+4 
...
class AlignPool:
    def __init__(self, tracks):
        ...
        self.pool = ctx.Pool(processes=self.tracks.conf.tracks.io.processes, maxtasksperchild=1)
...
def dtw_pool(conf):
    ...
    hdf5 = HDF5File(tracks.output.filename,tracks.output.prms)
    # tracks.output._init_hdf5_output(False)
    for chunk,counts in pool: #dtw_pool_iter(tracks):
        i += len(chunk)
        # tracks.output.write_buffer(chunk)
        for out in chunk:
            hdf5.write(out)
        speed = i/(time()-t)*60*60
        sys.stdout.write(f'Process {i} reads. Processing speed {speed} reads/hour\n')
        sys.stdout.flush()
